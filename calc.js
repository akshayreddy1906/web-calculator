...
function calculate() {
   ...
   let result;
   switch (operator) {
      case '+': result = (prev + current).toString(); break;
      case '-': result = (prev - current).toString(); break;
      case '*': result = (prev * current).toString(); break;
      case '/': result = (current === 0) ? (clearDisplay(), 'Error: Division by zero') : (prev / current).toString(); break;
      default: result = current.toString();
   }
   prevValue = currentValue;
   currentValue = '0';
   return result;
